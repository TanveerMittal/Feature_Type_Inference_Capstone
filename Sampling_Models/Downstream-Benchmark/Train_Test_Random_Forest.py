...
def Train_Test_Random_Forest(xtrain, xtest, truncated=False):
...
#     y_train
    ...
    def ProcessStats(data,y,truncated=False):
        ...
        if truncated:
                        data1 = data[['total_vals', 'num_nans', '%_nans', 'num_of_dist_val', '%_dist_val', 'mean', 'std_dev', 'min_val','max_val','has_delimiters', 'has_url', 'has_email', 'has_date', 'mean_word_count', 'is_list', 'is_long_sentence']]
    ...
    def FeatureExtraction(data,data1,flag,truncated=False):
    ...
    xtrain1 = ProcessStats(xtrain,y_train,truncated)
    xtest1 = ProcessStats(xtest,y_test,truncated)


    X_train = FeatureExtraction(xtrain,xtrain1,1,truncated)
    X_test = FeatureExtraction(xtest,xtest1,0,truncated)
